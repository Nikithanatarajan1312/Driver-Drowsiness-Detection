...
model = load_model("drowiness_new.h5")
count = 0
alarm_on = False
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        for (x3, y3, w3, h3) in mouth:
            cv2.rectangle(roi_color, (x3, y3), (x3 + w3, y3 + h3), (0, 255, 0), 1)
            mouth1 = roi_color[y3:y3 + h3, x3:x3 + w3]
            mouth1 = cv2.resize(mouth1, (145, 145))
            mouth1 = mouth1.astype('float') / 255.0
            mouth1 = img_to_array(mouth1)
            mouth1 = np.expand_dims(mouth1, axis=0)
            pred3 = model.predict(mouth1)
            status3=np.argmax(pred3)
            #print(status3)
            #status2 = classes[pred2.argmax(axis=-1)[0]]
            break
        ...
        if status1 == 2 and status2 == 2:
            ...
            count += 1
            cv2.putText(frame, "Yawning, Frame count: " + str(count), (10, 30), cv2.FONT_HERSHEY_COMPLEX, 1,
                        (0, 0, 255), 1)
            if count >= 2:
                cv2.putText(frame, "Drowsiness Alert!!!", (100, height-20), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
                if not alarm_on:
            ...
            cv2.putText(frame, "Eyes Open and Mouth Closed", (10, 30), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 1)
            count = 0
            alarm_on = False
